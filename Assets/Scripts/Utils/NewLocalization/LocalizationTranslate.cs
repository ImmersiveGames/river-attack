...
namespace MyUtils.NewLocalization
{
    public class LocalizationTranslate
    {
        ...
        public string Translate(LocalizationString localizationString, float qnt = 1, StringFormat format = StringFormat.None)
        {
            return GetStringFormat(GetText(localizationString, qnt), format);
        }
        private string GetText(LocalizationString localizationString, float qnt = 1)
        {
            EntryLocalization entry = GetEntry(localizationString);
            if (entry != null && entry.entrys.Length > 0)
            {
                int plural = PluralForm(entry.language.pluralForm, (int)qnt);
        ...
        public string GetStringFormat(string entry, StringFormat stringFormat)
        {
            switch (stringFormat)
            {
                ...
                case StringFormat.FirstLetterUp:
                    return MyUtils.Tools.FirstLetterToUpper(entry);

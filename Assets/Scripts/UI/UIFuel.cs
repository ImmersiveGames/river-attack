...
public class UIFuel : MonoBehaviour {
    ...
    private void RotatePointerUI()
    {
        ...
        {
            point = fuel.Value;
            float cento = Utils.Tools.GetCento(fuel.Value, pointFull);
            float centoDistancia = Utils.Tools.GetValorCento(cento, minMaxAngleRotation.maxValue - startRotation.eulerAngles.z);
